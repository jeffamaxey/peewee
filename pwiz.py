...
def make_introspector(database_type, database_name, **kwargs):
    if database_type not in DATABASE_MAP:
        err(f"Unrecognized database, must be one of: {', '.join(DATABASE_MAP.keys())}")
        sys.exit(1)
...
def print_models(introspector, tables=None, preserve_order=False,
    ...
    header = HEADER % (
        ...
        f', **{repr(db_kwargs)}' if db_kwargs else '',
    )
    print_(header)
    ...
    def _print_table(table, seen, accum=None):
        ...
        for foreign_key in foreign_keys:
            ...
            if dest in accum and table not in accum:
                print_(f'# Possible reference cycle: {dest}')
        ...
        print_(f'class {database.model_names[table]}(BaseModel):')
        ...
        for name, column in columns:
            ...
            if is_unknown and ignore_unknown:
                disp = f"{column.name} - {column.raw_column_type or '?'}"
                print_(f'    # {disp}')
            else:
                print_(f'    {column.get_field()}')
        ...
        if multi_column_indexes:
            ...
            for fields, unique in sorted(multi_column_indexes):
                print_(
                    f"""            (({', '.join(f"'{field}'" for field in fields)}), {unique}),"""
                )
            print_('        )')
        ...
        if len(primary_keys) > 1:
            ...
            pk_list = ', '.join(f"'{pk}'" for pk in pk_field_names)
            print_(f'        primary_key = CompositeKey({pk_list})')
    ...
    for table in sorted(database.model_names.keys()):
        if table not in seen and (not tables or table in tables):
            _print_table(table, seen)

def print_header(cmd_line, introspector):
    timestamp = datetime.datetime.now()
    print_('# Code generated by:')
    print_(f'# python -m pwiz {cmd_line}')
    print_(f"# Date: {timestamp.strftime('%B %d, %Y %I:%M%p')}")
    print_(f'# Database: {introspector.get_database_name()}')
    print_(f'# Peewee version: {peewee_version}')
    print_('')
